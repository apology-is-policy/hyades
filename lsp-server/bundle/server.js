...
// dist/hyades-wasm.js
var require_hyades_wasm = __commonJS({
  "dist/hyades-wasm.js"(exports2) {
    "use strict";
    var __createBinding = exports2 && exports2.__createBinding || (Object.create ? function(o, m, k, k2) {
      if (k2 === void 0)
        k2 = k;
    ...
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.HyadesWasm = void 0;
    var path = __importStar(require("path"));
    var fs = __importStar(require("fs"));
    var HyadesWasm = class _HyadesWasm {
      constructor(module3, initOutput = []) {
        this.module = module3;
      ...
      static async initialize() {
        const jsPaths = [
          path.join(__dirname, "hyades-compat.js"),
          // Same dir (bundled)
          path.join(__dirname, "..", "hyades-compat.js"),
          // Parent dir (unbundled dist/)
          path.join(process.cwd(), "hyades-compat.js")
          // Current working dir
        ];
        let jsPath = null;
        ...
        if (!jsPath) {
          ...
          return new _HyadesWasm(mockModule);
        }
        try {
          const createModule = require(jsPath);
          const wasmOutput = [];
          const wasmDir = path.dirname(jsPath);
          const module3 = await createModule({
            locateFile: (filename) => path.join(wasmDir, filename),
            // Redirect stdout/stderr to avoid corrupting LSP stdio
            print: (text) => wasmOutput.push(text),
            printErr: (text) => wasmOutput.push(`[WASM err] ${text}`)
          });
          module3._wasmInitOutput = wasmOutput;
          if (module3._hyades_wasm_init) {
            module3._hyades_wasm_init();
          }
          return new _HyadesWasm(module3, wasmOutput);
        } catch (err) {
...
`);
    ...
    exports2.HyadesWasm = HyadesWasm;
    function createMockModule() {
      ...
      return {
        ...
        _hyades_lsp_get_completions: (_line, _col) => {
          const completions = [
        ...
        _hyades_lsp_get_definition: (_line, _col) => {
          const result = "null";
          const ptr = nextPtr++;
          strings.set(ptr, result);
          return ptr;
        },
        _hyades_lsp_get_hover: (_line, _col) => {
          const result = "null";
          const ptr = nextPtr++;
          strings.set(ptr, result);
          return ptr;
        },
        _hyades_lsp_get_references: (_line, _col) => {
          const result = JSON.stringify([]);
          const ptr = nextPtr++;
          strings.set(ptr, result);
          return ptr;
        ...
        _hyades_lsp_free: (ptr) => {
          strings.delete(ptr);
        },
        _hyades_lsp_clear: () => {
        },
        _hyades_lsp_has_result: () => 1,
        _malloc: (size) => {
          const ptr = nextPtr++;
          strings.set(ptr, "");
          return ptr;
        ...
        stringToUTF8: (str, outPtr, _maxBytes) => {
...
var hyades_wasm_1 = require_hyades_wasm();
...
var tokenTypes = [
  ...
  // 6
  "parameter",
  // 7 - Macro parameters ${name}
  "variable",
  // 8 - References, counters
  "property",
  // 9
  "enumMember",
  // 10
...
connection.onInitialize(async (params) => {
  ...
  try {
    hyades = await hyades_wasm_1.HyadesWasm.initialize();
...
async function validateDocument(document) {
  ...
  try {
    ...
    const diagnostics = lspDiagnostics.map((diag) => ({
...
connection.onDefinition((params) => {
  ...
  if (!definition)
    return null;
  return node_1.Location.create(params.textDocument.uri, definition.range);
});
...
connection.onReferences((params) => {
  ...
  return references.map((ref) => node_1.Location.create(params.textDocument.uri, ref.range));
});
...
connection.languages.semanticTokens.on((params) => {
  try {
    ...
    const validData = tokens.data.map((v) => Math.max(0, Math.floor(v)));
